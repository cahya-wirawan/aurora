...
**The custom widget toolkit** (`aurora-widgets`) and design-token system (`aurora-theme`) are what the app's own UI is built from: thirteen widget kinds (the nineteen `WidgetKind` variants minus the plain `Container` and the five child-only parts — `ListRow`, `DropdownList`, `Tab`, `Tooltip` and `MenuSeparator`, `Tooltip` being the node a caller-held `Tooltip` controller inserts under its owner while shown, which is why the tooltip counts as one of `widgets/mod.rs`'s "10 of the 12 named widgets" — measured against the design gallery's own list — but not as a widget kind here, while `Menu` counts as both) with real layout, focus, hit-testing, and IME composition, all thirteen with real GPU-rendered paint (the child-only parts are painted too) through `aurora-vector`'s tessellation (verified with real pixel-readback tests; a separate vertex-coloured gradient pipeline also exists for the upcoming colour picker, though no widget paints a gradient yet), six named widgets additionally covered by a *blessed* golden-image gallery harness (six more named widgets — Scrollbar, Tree, Dropdown, Tab bar, Tooltip and Menu, the Tooltip being a named widget but not one of the thirteen kinds — have the same per-theme gallery coverage but their own goldens are still unblessed, pending a human on real GPU hardware). All five of PRD.md's built-in themes exist as real, verified design files, not just names: Dark, Light, High Contrast Dark, High Contrast Light, and a neutral-grey Colour-Critical theme, each automatically checked against the same 17 WCAG 2.1 AA contrast pairs (Colour-Critical additionally has its surfaces checked for genuine chroma neutrality — an automated satisfaction of the PRD's own acceptance criterion for that theme, not just "we picked hex codes that look gray"). The six blessed widgets' 30 goldens (six widgets × five themes) are committed and run on every GPU test pass; what's still outstanding is a human reviewing and blessing the other 30 — exactly the 30 `#[ignore]`d golden-diff tests in `crates/aurora-widgets/tests/gallery.rs` (Scrollbar, Tree, Dropdown, Tab bar, Tooltip and Menu × five themes), none of which has a committed PNG yet — on real GPU hardware before they're trusted as regression baselines. Themes are never blessed blind.
