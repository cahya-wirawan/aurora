...
| R2f | Solo design owner (FR-027 *Ownership*) — no second opinion in review, and the same person carries engineering decisions, so design work can be squeezed under schedule pressure | Medium (mitigated 2026-07-28) | Fix the token vocabulary in Phase 0 and treat additions as deliberate changes; seek outside critique on the mockups before they harden — **done**: a colleague reviewed and signed off as fine for a start, revisable later |
...
1. **Accessibility conformance target** — WCAG 2.1 AA equivalent, or a specific procurement standard (Section 508 / EN 301 549)? This sets the bar the §9 Phase 1 audit measures against, and custom UI means it is earned widget by widget.
    ...
    **Resolved 2026-08-04: WCAG 2.1 AA's success criteria, reinterpreted per criterion for desktop software** — see [ADR 0006](docs/adr/0006-accessibility-conformance-target.md) for the full reasoning. Chosen because it's the substantive floor both named procurement standards already build on (Section 508's 2017 refresh applies WCAG 2.0 AA to non-web software; EN 301 549 — the basis for the EU's 2025 European Accessibility Act — is built on WCAG 2.1 AA), not a claim of formal conformance to either. Consistent with, and extends, FR-027's own already-shipping WCAG 2.1 AA contrast requirement rather than inventing a second bar. The Phase 1 audit this question asks about now has a concrete checklist to measure against — see PLAN.md 0.1 and M1.10.
