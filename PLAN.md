...
- [~] **Basic brush and eraser (real engine is Phase 2)** — first slice
  ...
  **Still open**: eraser (same `stamp_dab` math, transparent instead of
  ...
  layer. Painting also has no visual feedback yet — nothing in this
  crate renders a document's pixels to the screen at all (M1.8's own
  still-open canvas-rendering bullet) — so a real run paints into a real
  tile store with no way to see it happen; that's the next real gap
  this closes toward, not one this step could reach.
