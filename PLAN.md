...
**Updated 2026-08-12 — this paragraph was frozen at the M1.1/M1.2
boundary (2026-07-28/29) for two weeks of real work; every sentence
below reflects the actual current state, not that snapshot.**

**Phase 0 (technical de-risking)** — its Phase-1 gate (PRD §13 Steps
1/3/4) was satisfied 2026-07-28 and remains so. Since then, the vertical
slice was re-run at the real 300,000 px PSB ceiling (0.3: paging
behaviour came back identical to the byte against the smaller
100,000 px baseline, confirming the sparse tile store's cost tracks
tiles actually touched, not document size). Still genuinely open, all
requiring hardware this session doesn't have: Windows/DX12 validation,
the Linux/Windows human legs of the accessibility/IME checklist, and
macOS/Windows LGPL packaging. PSD format coverage (masks, smart
objects, layer styles, compression variants) also remains open —
blocked in this specific sandbox by a missing `pip`/`psd-tools`
install, not a design question.

**Phase 1 (document, canvas, layers, rendering, shell) — substantial,
real progress across every M1.x milestone, but "done" varies by
section; checked directly against this file's own `[x]`/`[~]`/`[ ]`
marks, not assumed.** Only **M1.1** (core/tile-store) and **M1.6**
(design system, all five built-in themes) have every top-level item
at `[x]`. **M1.2** (GPU layer), **M1.3** (render graph/renderer),
**M1.4** (document model), **M1.5** (colour), and **M1.7** (widget
toolkit) each have most items `[x]` with one or two genuine `[~]`
items named in their own sections (respectively: DX12/Windows
validation; progressive rendering's and async evaluation's real
consumers; the crash-recovery journal's durable disk persistence;
working-space policy; vector-first rendering and per-theme/density
gallery coverage). **M1.9** (basic tools and I/O) has the largest
raw count of both `[x]` (20) and `[~]` (7) items — the `[~]` items
predate much of this session's later compositing work and may be
understated relative to it, worth a fresh look rather than trusted
blindly. **M1.8** (application shell) is the outlier: every one of
its ten top-level items is still `[~]`, despite substantial, real,
...
- [x] **Built-in themes: Dark, Light, 2× high contrast, Colour-Critical**
  ...
  *Ownership*) for Light specifically, made the colour calls himself, and
  engineering implemented and wired them — `design/themes/light.toml`
  (`extends = "Dark"`), plus three new dark-end `accent.blue` steps
  (100/200/300) added to `design/tokens/palette.toml` for Light's
  dark-fill/light-text button pattern (Dark's own 400/500/600 untouched).
  Contrast-verified by hand against the real WCAG 2.1 formula and by a
  dedicated test, `contrast::tests::the_real_light_theme_passes_every_
  gated_pair` (registers both `dark.toml` and `light.toml` into one
  `ThemeSet` so Light's `extends` chain resolves, then asserts all 17/17
  gated pairs pass) — same pattern as the existing Dark test, no parser
  changes needed. **Still blocked on Cahya designing the remaining two**
...
- [~] **Component gallery + golden-image tests per theme and density** —
  ...
  **Extended to every painted widget, 2026-08-07** — `Checkbox`,
  `Slider`, `TextField`, and `CommandPalette` (added the same day each
  gained real paint, once `Button`'s own golden was blessed and the
  harness itself proven real) each got their own `*_gallery_tree`, own
  `render_gallery` call, and own golden PNG target, exactly the "add a
  tree, call `render_gallery`, bless a golden" shape this bullet's own
  doc comment already predicted — genuinely not new harness work,
  `render_gallery`/`collect_gallery_paints`/`draw_gallery_paints`/
  `read_rgba8` were all already generic over `tree`/`size`/`clear` and
  needed zero changes. `sized_style`/`sample_cell_centre` widened to
  take a cell size parameter instead of the one hardcoded `CELL`
  constant (renamed `BUTTON_CELL` for consistency with the new
  per-widget constants); new `sample_at` for an arbitrary absolute
  pixel, not just a cell's own centre.

  Each new gallery picked its own cell size as a multiple of 64px (so
  `width * 4` stays a multiple of `wgpu::COPY_BYTES_PER_ROW_ALIGNMENT`
  regardless of cell count — the same constraint `read_rgba8`'s own
  doc comment already named): `Checkbox` mirrors `Button`'s own
  64×64/three-cell shape; `Slider` is wider than tall (128×32) since a
  thumb's own position needs real horizontal travel to mean anything;
  `TextField` 192×32/two cells (enabled, disabled — no pressed-style
  third state exists for it); `CommandPalette` is a single 192×128
  cell, not a row, since `paint_widget` resolves exactly one visual
  state for it today.

  `Slider`'s own self-contained "distinct pixels" test needed a
  genuinely different shape from the others': sampling each cell's own
  *centre* would just show track for anything but a dead-centre value,
  so it samples a fixed offset from each cell's own left edge instead —
  at the minimum value that offset is inside the thumb
  (`accent.primary`), at the maximum value the thumb has moved away and
  that same offset now shows bare track (`surface.sunken`) — a real
  proof the thumb's own rendered *pixels* moved, not just its mesh
  vertices (`src/paint.rs`'s own unit test already covers the
  geometry). `CommandPalette` has only one visual state, so its own
  self-contained test doesn't compare two states against each other
  the way every other widget's does — it just proves the panel's own
  centre isn't still showing the black clear colour underneath it.

  10 tests total in `gallery.rs` now (was 2): 6 real, self-contained
  "distinct pixels" tests, all passing here; 4 golden-diff tests
  (`checkbox`/`slider`/`text_field`/`command_palette_gallery_matches_
  the_golden_image`), all `#[ignore]`d the same "never bless blind"
  way `button_gallery_matches_the_golden_image` itself was until
  blessed — each needs the same human-on-real-GPU-hardware bless step
  `Button`'s own golden already went through.

  Verified: `cargo fmt -p aurora-widgets --check`, `cargo clippy -p
  aurora-widgets --all-targets --all-features -- -D warnings`, `cargo
  test -p aurora-widgets` (160 lib tests unchanged, `gallery.rs`: 6
  passed + 4 ignored, `headless.rs` unchanged) — all clean. Checked by
  hand against `scripts/check_no_hardcoded_style.py`'s own two
  patterns too, though moot here — that script only scans `src/`, and
  this is all `tests/`.

  **Still genuinely open**: four real bless steps waiting on Cahya
  (same as `Button`'s own, done above); per-theme and per-density
  coverage (today: Dark only, one density implicitly — `paint_widget`
  itself doesn't yet vary by density since only spacing/radius tokens
  feed it, not a density axis); matching
  [design/gallery/index.html](design/gallery/index.html)'s full
  per-widget-per-state review surface (hover/focus states, every
  built-in theme — most of which don't exist yet either); and
  `CommandPalette`'s own still-open query-text/row-highlighting gap
  (`src/paint.rs`'s own doc comment), which this gallery extension
  doesn't attempt to work around.

  **A real finding from Cahya's own bless run, 2026-08-07 — a
  usability bug in the harness, not the paint code**: Cahya blessed
  all four goldens on real hardware, but `command_palette_gallery.png`
  came back looking like "just a black image." Checked by actually
  decoding the committed PNG's own raw pixel bytes (a throwaway
  scratchpad crate using the `png` crate directly — not by eye, not
  assumed): the panel genuinely is `[40, 40, 44]`, exactly
  `surface.raised` (`#28282c`) — correct data. The problem is real
  but different: `surface.raised`/`surface.sunken` are deliberately
  near-black by design (FR-027's own "near-neutral" principle), and
  against this gallery's own plain `wgpu::Color::BLACK` backdrop that
  leaves nothing for a human to visually anchor against, especially
  with the panel filling almost the entire frame. Decoded
  `text_field_gallery.png` too, on the same suspicion: `[20,20,20]`
  enabled / `[8,8,8]` disabled — same root cause, arguably worse (no
  bright reference colour anywhere in that image at all). Decoded
  `checkbox_gallery.png` for comparison: unchecked/disabled are
  equally dark, but the checked cell's own `[120,172,255]`
  (`accent.primary`) gives a real reference point, so that one's fine
  as blessed.

  **Fixed, with Cahya's own explicit choice of remedy** (asked rather
  than assumed, since it's a visual-design tradeoff, not a pure
  engineering one): new `NEUTRAL_CLEAR` (a mid-grey
  `wgpu::Color`), used only for `CommandPalette`/`TextField`'s own
  `render_gallery` calls — `Button`/`Checkbox`/`Slider` keep plain
  black, already blessed, already real reference points within each
  of those images, no reason to force an unnecessary re-bless.
  `render_gallery_produces_the_command_palettes_own_panel`'s own
  assertion was also made more robust while this was open anyway: it
  used to hardcode `!= [0, 0, 0, 255]`, which would have been wrong the
  moment the clear colour changed; it now compares the panel's own
  centre against its own corner pixel `(0, 0)` instead —
  `scales.radius.md`'s real rounded corner means that corner is
  provably outside the filled shape regardless of what colour fills
  it, so the comparison stays correct no matter what backdrop a future
  change picks.

  The two now-wrong goldens (blessed against the old black backdrop,
  provably correct data but a different image than `NEUTRAL_CLEAR`
  now produces) were removed (`git rm`) rather than left stale in the
  repo — a committed golden that silently no longer matches what the
  code renders is worse than no golden at all. Both tests are
  `#[ignore]`d again, waiting on a fresh bless from Cahya.

  Verified: `cargo fmt -p aurora-widgets --check`, `cargo clippy -p
  aurora-widgets --all-targets --all-features -- -D warnings`, `cargo
  test -p aurora-widgets` (160 lib tests unchanged, `gallery.rs`: 8
  passed + 2 ignored — `Checkbox`/`Slider` un-ignored now that they're
  confirmed correct and reviewable, `headless.rs` unchanged) — all
  clean.

  **`text_field_gallery.png`'s own re-bless, 2026-08-07, confirmed
  fine as-is**: Cahya re-ran the bless command and committed it;
  decoded its raw bytes too — enabled `[20,20,20]`, disabled
  `[85,85,85]`, genuinely distinct (`TextField` has two states to
  contrast *against each other*, unlike `CommandPalette`'s one, so it
  never needed a margin fix the way that one did). Un-ignored.

  **`command_palette_gallery.png`'s own re-bless came back "still
  looks dark," a second real bug in the harness, found only because
  Cahya actually looked again**: `NEUTRAL_CLEAR` alone wasn't the
  whole fix. Decoded the new PNG's own bytes to confirm before
  guessing: the backdrop genuinely was the new grey
  (`[127,127,127]`, visible at the four extreme corners) — the clear-
  colour fix worked exactly as intended — but the panel still filled
  essentially the entire frame with no margin at all
  (`COMMAND_PALETTE_GALLERY_SIZE` equalled `COMMAND_PALETTE_CELL`
  exactly), so `scales.radius.md`'s own tiny rounded corners were the
  *only* pixels ever showing the backdrop — invisible at normal
  review size regardless of how much contrast the colour change
  itself added.

  Fixed with a real `COMMAND_PALETTE_MARGIN` (32px, new) on every side
  of the panel, via a new `command_palette_style` (`size` plus
  `margin`, both through `taffy::style_helpers::length`, confirmed to
  work for `margin: Rect<LengthPercentageAuto>` the same way this
  file's own `sized_style` already used it for `Size<Dimension>`) —
  `COMMAND_PALETTE_GALLERY_SIZE` grew to `CELL + 2 × MARGIN` on each
  axis (256×192, still a multiple of 64). Root cause of why a bigger
  `render_gallery` target alone wouldn't have been enough on its own:
  the tree's own root (`new_tree`'s default `Style::default()`, `Auto`-
  sized) shrink-wraps to its one child's own margin box — the standard
  flexbox content-sizing behaviour — so only giving the *child* real
  margin, not just enlarging the render target, actually pushes the
  panel's own computed bounds inward.

  **Verified the layout half headlessly before asking for a third
  bless, rather than just hoping**: new
  `command_palette_style_positions_the_panel_with_a_real_margin` is a
  plain CPU test (`WidgetTree::compute_layout`/`::bounds`, no GPU
  needed at all) proving the panel's own real bounds sit inset by
  exactly `COMMAND_PALETTE_MARGIN` on every side, and that panel +
  margins exactly fill the gallery frame — this sandbox can't prove
  the actual rendered pixels look right (no GPU adapter), but it can
  prove the *geometry* the fix depends on is correct, real confidence
  gained without another round-trip through a human's own eyes.
  `render_gallery_produces_the_command_palettes_own_panel`'s own
  centre-sample also had to move, since `sample_cell_centre` assumes
  cell 0 starts at `x = 0` — no longer true once the panel has its own
  margin offset.

  The now-stale golden (blessed against the no-margin layout, correct
  data for that layout but not what the code renders anymore) was
  removed again, same "don't leave a silently-wrong file committed"
  reasoning as the first round. Still `#[ignore]`d, waiting on a third
  bless.

  Verified: `cargo fmt -p aurora-widgets --check`, `cargo clippy -p
  aurora-widgets --all-targets --all-features -- -D warnings`, `cargo
  test -p aurora-widgets` (160 lib tests unchanged, `gallery.rs`: 10
  passed + 1 ignored, `headless.rs` unchanged) — all clean.

  **`command_palette_gallery.png`'s third bless confirmed correct,
  2026-08-07**: Cahya re-ran the bless command against the margin-
  corrected layout and reported it "looks better now." Decoded the new
  PNG's own raw bytes to confirm rather than trust the preview alone:
  backdrop `[127,127,127]` (`NEUTRAL_CLEAR`) at both extreme corners and
  one pixel *outside* the panel's own left edge (`x = 31`), panel
  `[40,40,44]` (`surface.raised`) at its own centre and one pixel
  *inside* that same edge (`x = 33`) — a real, visible margin on every
  side this time, not just tiny rounded-corner slivers. Un-ignored
  `command_palette_gallery_matches_the_golden_image`, doc comment
  rewritten to the same "Blessed and reviewed" style the other four
  goldens already use. `gallery.rs`: 11 passed, 0 ignored — every
  painted widget's golden is now confirmed and reviewed. Verified:
  `cargo fmt --all --check`, `cargo clippy -p aurora-widgets
  --all-targets --all-features -- -D warnings`, `cargo clippy
  --workspace --all-targets --all-features -- -D warnings`, `cargo test
  --workspace` (every crate green) — all clean (`check_layering.py`/
  `check_no_hardcoded_style.py` unrunnable in this sandbox, no
  `python3`, as throughout this session).

  **Still genuinely open**: extending the gallery to per-theme/per-
  density coverage; matching `design/gallery/index.html`'s full per-
  widget-per-state review surface; `CommandPalette`'s own result-row
  highlighting (tree walk vs. a new `WidgetKind` variant — a real
  architecture decision for Cahya, not to be picked unilaterally); the
  other 9 named widgets' paint.

  **`ColorSwatch` added, 2026-08-07** — a seventh `WidgetKind`
  (`widgets::color_swatch`, `Role::ColorWell`), picked from
  `design/gallery/index.html`'s own 12-widget list at Cahya's own
  direction (asked which open thread to pick up next rather than
  guessing) as the simplest remaining one: one shape, no real text
  shaping needed, unlike `Dropdown`/`Tab bar`/`Tooltip`. Genuinely
  different from every other widget painted so far: the colour a swatch
  shows (`ColorSwatchState::color`) is arbitrary caller data — the
  document's current foreground/background colour, one entry in a
  colour picker — not a `Theme` token, the same "content, not chrome"
  distinction `TextField::content` already established; `paint_color_
  swatch` still resolves `scales.radius.sm`/`state.disabled_opacity`
  from real tokens, since those *are* chrome, not content. Sized via
  the exact same "no dedicated control-size token exists yet" grounding
  `Checkbox`'s own `style` already uses (`type_size(scales.typography.
  size.md)`) rather than the design mockup's own hardcoded 32px, which
  is a literal in a static CSS file, not a resolved token. 7 new
  `aurora-widgets` lib tests (167 total): 5 in `widgets::color_swatch`
  (insert/mutate-colour/disable, the same shape `checkbox.rs`'s own
  tests already use), 2 in `paint.rs` (paints its own arbitrary colour
  at full opacity; disabled dims that same colour via
  `disabled_opacity`, not a token colour). Gallery harness extended:
  `color_swatch_gallery_tree` (two visually distinct arbitrary colours
  plus a disabled repeat of the first, specifically chosen so the
  distinct-pixels test can prove "two different colours render
  differently" *and* "the same colour disabled still dims," not just
  one or the other), a real self-contained distinct-pixels test (passes
  here), and a golden-diff test — `#[ignore]`d pending a human bless on
  real GPU hardware, the same "never bless blind" discipline every
  other golden here follows. `gallery.rs`: 12 passed + 1 ignored.
  Verified: `cargo fmt --all`, `cargo clippy -p aurora-widgets
  --all-targets --all-features -- -D warnings`, `cargo clippy
  --workspace --all-targets --all-features -- -D warnings`, `cargo test
  --workspace` (every crate green) — all clean (`check_layering.py`/
  `check_no_hardcoded_style.py` unrunnable in this sandbox, no
  `python3`; checked the new files by hand for hex-colour/`length
  (<literal>)` violations instead, same as every prior widget addition
  this session).

  **Still genuinely open**: `color_swatch_gallery.png`'s own bless
  step; per-theme/per-density gallery coverage (blocked on Light/both
  high-contrast/Colour-Critical actually existing as real, owner-
  approved themes — `aurora-theme`'s own doc comment: "Only the Dark
  theme exists as a real design," not this crate's decision to invent
  the others); matching `design/gallery/index.html`'s full per-widget-
  per-state review surface; `CommandPalette`'s own result-row
  ...
  **`Slider` gained the same Light-theme slice too, 2026-08-09**
  (third widget, same one-at-a-time pattern): two new tests reusing
  `slider_gallery_tree` unchanged, `light_theme()`/`LIGHT_CLEAR` (both
  already existed from `Button`'s own slice above, not redefined).
  `render_gallery_produces_distinct_pixels_for_each_slider_state_in_
  light_theme` is real and self-contained (no golden), sampling the
  same fixed offset from each cell's own left edge the Dark version
  already established. `slider_gallery_matches_the_golden_image_in_
  light_theme` is the real golden-diff test, targeting `tests/golden/
  slider_gallery_light.png` (does not exist yet, not created here) —
  `#[ignore]`d for the same "never bless blind" reason as `Button`'s
  and `Checkbox`'s own; a human on real GPU hardware still needs to run
  `AURORA_BLESS_GOLDEN=1 cargo test -p aurora-widgets --test gallery --
  --ignored`, open the written PNG, and confirm it actually shows the
  thumb at three visually distinct positions/opacities in Light-theme
  colours before that attribute comes off. Checked, not guessed,
  whether `Slider` needed a `NEUTRAL_CLEAR`-style backdrop fix the way
  `CommandPalette`/`TextField` did: Light's `surface.sunken`
  (`neutral.700`, `#c1c1c7`) against `LIGHT_CLEAR` (`neutral.900`,
  `#f5f5f6`) is a real, if modest, contrast, and the thumb's own bright
  `accent.primary` (`#124fb0` in Light) gives a reviewer a genuine
  reference point regardless — the same shape `Checkbox`'s own modest
  unchecked-box contrast already gets away with — so no new backdrop
  constant was added. Ran `cargo test -p aurora-widgets --test gallery
  -- --nocapture` in this sandbox and grepped its own output for
  `SKIPPED`: zero matches, so this sandbox had a real (possibly
  software) GPU adapter and the render path genuinely executed — 17
  passed, 3 ignored (all three Light golden-diffs: `Button`'s,
  `Checkbox`'s, `Slider`'s), 0 failed. `cargo fmt --all --check` and
  `cargo clippy -p aurora-widgets --all-targets --all-features -- -D
  warnings` both clean; `python3 scripts/check_no_hardcoded_style.py`
  clean (25 files scanned, this sandbox does have a working `python3`
  this time); `check_layering.py` still fails in this sandbox with the
...
- [~] **Canvas: infinite zoom, rotation, pan, rulers, guides, grid,
  ...
  **Sub-tile fractional scroll fixed, 2026-08-13 — two real symptoms
  Cahya reported interactively, one root cause, closing the gap this
  bullet's own "still open" paragraph above named**: while testing on
  real hardware, painted content landed offset from the cursor after
  zooming or panning, and while dragging with the Pan tool the canvas
  often didn't visibly move at all until a fairly large drag, then
  jumped. Same root cause, described from two angles: `aurora_gpu::
  TileResidency::set_origin` only ever accepted a whole-tile `TileId`,
  and `aurora-app`'s `tile_origin_for_view` (now renamed
  `canvas_local_origin`, since it no longer returns a tile) floored its
  own exact, continuous document-space position to that `TileId` before
  handing it over — discarding the fractional remainder within that
  tile entirely, so the rendered atlas always snapped to the nearest
  tile boundary rather than the true position pointer math already used
  for painting. Invisible at the default view (zoom 100%, no pan, layer
  at document origin) since that position happens to be exactly
  tile-aligned; any real zoom-anchor pan or plain drag generally isn't.
  Fixed by threading fractional precision through `TileResidency`
  itself rather than patching the one caller: `set_origin`'s second
  parameter is now a continuous `doc_origin: (f32, f32)`, clamped to
  non-negative, split internally into the whole-tile `TileId` its own
  slot addressing still needs and a new `sub_tile: (f32, f32)` field
  (the remainder within that tile); `write_uniform` folds both into the
  atlas's own sampled UV offset (`scroll = origin * TILE + sub_tile`,
  wrapped with `f32::rem_euclid` for the toroidal addressing — the safe
  choice even though the upstream clamp means `scroll` can't actually go
  negative today). `canvas_local_origin` now returns the raw, unfloored,
  possibly-negative local position and lets `set_origin` do the
  flooring/clamping — the crate that actually needs whole-tile
  addressing for its own bookkeeping, not a caller pre-committing to it.
  3 new real-GPU pixel-readback tests in `aurora-gpu`
  (`render_test.rs`): `canvas_pipeline_reflects_a_sub_tile_fractional_pan`
  (a half-tile `doc_origin` shifts the rendered tile boundary by exactly
  that many pixels, not snapped to the nearest whole tile — the money
  test), `canvas_pipeline_with_a_whole_tile_doc_origin_matches_the_pre_refactor_tileid_behaviour`
  (regression backstop: an exact-`TILE`-multiple `doc_origin` renders
  bit-identically to the old `TileId`-typed API), and
  `canvas_pipeline_a_small_sub_tile_pan_visibly_moves_the_content` (a
  50px pan on a 256px tile — smaller than one full tile — visibly
  changes the rendered pixel, proving the "panning doesn't move
  anything" symptom specifically, not just that the fractional math
  computes correctly in isolation). `aurora-gpu`'s own existing
  `residency.rs` tests and `aurora-app`'s `canvas_local_origin_*` tests
  (formerly `tile_origin_for_view_*`) updated for the new signature; two
  of the latter (`..._follows_a_negative_pan`,
  `..._accounts_for_zoom_not_just_pan`) turn out to have exercised a
  genuinely fractional pan/zoom scenario whose old, floored assertion
  (`TileId { 1, 1 }`) was only the whole-tile *part* of the true answer
  (300.0, 300.0) — itself a small piece of evidence this bug was real,
  not hypothetical. Atlas margin re-checked, not assumed: `TileResidency
  ::new`'s existing "+1 tile of margin" grid sizing already covers the
  worst case (`sub_tile` up to just under one `TILE`) at zoom >= 1 —
  `n * TILE >= viewport_px` by construction (`n = ceil(viewport_px /
  TILE)`), so `(n + 1) * TILE > viewport_px + sub_tile` always, with
  room to spare — no atlas resize needed. (Zoom < 1, i.e. zoomed out
  past 100%, can already need more texels than the fixed-size atlas
  holds — a separate, pre-existing limitation of the shader-side-only
  `uv_scale` design, not something this fix changes or was asked to
  address.) Still explicitly out of scope: rotation, rulers, guides,
  grid, snap, and true infinite zoom — exactly as this bullet's own
  "still open" paragraph already said, minus the sub-tile item it just
  named (window resize was already handled in an earlier round this
  session, `TileResidency::resize` — not reopened here). Verified:
  `cargo fmt --all --check`, `python3 scripts/check_layering.py`,
  `python3 scripts/check_no_hardcoded_style.py`, `cargo clippy
...
(2026-08-12) found more open, non-hardware, non-design-gated work than
an earlier draft of this same update assumed.** Only M1.1 and M1.6
have every top-level item at `[x]`; M1.2 through M1.9 all retain real
...
1. **Needs hardware this session doesn't have.** Windows/DX12
   validation (0.3), the Linux/Windows human legs of the
   accessibility/IME checklist (0.4, M1.10), macOS/Windows LGPL
   packaging (0.6), and re-verifying the 60 FPS frame-timing finding
   (M1.9/M1.10) on real GPU hardware — this sandbox and every subagent
   worktree sandbox used this session have only Mesa llvmpipe software
   Vulkan, confirmed directly via `GpuContext::adapter_info()`, not
   assumed.
...
3. **Blocked by this sandbox's own missing tooling, not a design or
   ...
   precision/colour-mode/PSB variants) needs `psd-tools`
   (Python) as an independent reader — this sandbox has no `pip`, so
   the spike's own established "never trust a writer's own reader
   alone" discipline can't be honestly satisfied here right now.
4. **Real, substantial, multi-round engineering work, deliberately not
   started without checking scope first.** Two gaps named "still
   genuinely open" by already-landed M1.9 rounds: extending GPU
   compositing beyond the Normal-blend/non-grouped case (a full WGSL
...
**The single most durable open risk, unchanged since Phase 0**: the
Linux/Windows human legs of the accessibility/IME checklist (bucket 1
above) remain the one thing that could still overturn
[ADR 0001](docs/adr/0001-custom-wgpu-ui.md)'s custom-`wgpu`-UI decision
— macOS passing (9/10, [full results](spike/a11y-ime/FINDINGS.md)) says
nothing about the other two platforms' own, entirely different
accessibility APIs (UIA, AT-SPI). Cahya accepted this as a risk rather
than a blocker for starting Phase 1 (2026-07-28) — still open, not
resolved, just not gating.
