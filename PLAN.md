...
- [~] **Import/export PNG, JPEG, TIFF** — PNG and JPEG done 2026-08-06;
  TIFF still open. `aurora-io`'s first real code (was a placeholder). Asked
  ...
  **Still `[~]`, not `[x]`**: TIFF remains unimplemented (its own
  dependency decision); no wiring into a real document (no layer owns
  pixel storage yet, see above); no ICC-profile-aware import/export for
  either format (always sRGB); no 16-bit export for PNG; no user-facing
