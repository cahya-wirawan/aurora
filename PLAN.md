...
- [~] **Undo/Redo, wired to a real, live `History`** — done 2026-08-06.
  ...
  **Still `[~]`, not `[x]`**: a completed Move's own undo granularity
  ...
  `Undo`/`Redo` are shortcut-only, matching how the existing tool-switch
  letters work — neither is in the command palette or the (macOS)
  native menu yet, since this crate has no Edit menu at all.
...
- [~] **Pixel-edit undo (Brush/Eraser)** — done 2026-08-06, in two
  ...
  **Still `[~]`, not `[x]`**: the "pixel-first, not unified" undo
  priority named above; a completed stroke's own undo granularity is
  the whole stroke as one step (unlike Move's own per-event
