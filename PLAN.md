...
- [~] **Move, marquee select, zoom, pan, eyedropper** — three of five
  ...
  **Still not wired into `aurora-app`, and not a small remaining
  step**: checking what actually *shows* a moved layer surfaced a
  second, real gap. `App::redraw`'s own `tile_origin_for_view` computes
  where to point the GPU atlas purely from `CanvasView`'s pan/zoom —
  it never reads the active layer's own `bounds.x`/`bounds.y` at all,
  because every layer built so far (`demo_document`, `open_file`'s
  `document_from_image`) sits at document `(0, 0)`, so nothing has ever
  needed to. Wiring `Drag::Move` through to `LayerTree::set_bounds`
  without also teaching the renderer to offset by the active layer's
  own bounds would be a real, user-visible break, not a working
  feature: the document model would say a layer moved, but the exact
  same pixels would keep rendering in the exact same place on screen —
  worse than not building it, since it would look silently broken
  rather than honestly absent. Flagged here rather than shipped
  half-working (CLAUDE.md: no half-finished features) — the actual
  interactive Move tool needs that renderer change too, real, separate,
  still-open follow-on work now scoped more accurately than "just UI
  wiring."
