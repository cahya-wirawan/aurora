...
- [~] **Undo/Redo, wired to a real, live `History`** — done 2026-08-06.
  ...
  **Still `[~]`, not `[x]`**: `App::paint_dab`/`App::erase_dab` still
  ...
  ops are tree-level changes, never pixel snapshots), so a stroke isn't
  undoable today; giving pixel edits their own undo needs a real,
  separate design (a pixel-level op storing dirtied tiles, per
  invariant §7.3.3), not invented here. A completed Move's own undo
  granularity is one step per pointer-move event during the drag, not
  one step per whole drag gesture — `apply_move` runs, and records,
  once per `CursorMoved` while dragging, so undoing several times
  during what felt like one drag steps back through its own
  intermediate positions rather than jumping straight to where it
  began; coalescing a drag into a single undo step is separate,
  still-open follow-on work. `Undo`/`Redo` are shortcut-only, matching
  how the existing tool-switch letters work — neither is in the command
  palette or the (macOS) native menu yet, since this crate has no Edit
  menu at all.
- [~] **Multi-layer compositing** — done 2026-08-06. The canvas had
  ...
  **Still `[~]`, not `[x]`, two real limitations named in code as well
  as here**: recompositing assumes every visible layer shares the same
  document-space origin as the atlas's own current view — true of every
  document this app's own UI can actually build today (image import
  always creates one layer at `(0, 0)`; `demo_document`'s three layers
  all share that origin too; only a hand-crafted `.aur` file could
  actually diverge two layers' origins, since Move is the only thing
  that ever changes one) — a real per-layer-origin-aware compositor
  (converting a document-space tile into each layer's own local tile,
  possibly spanning a sub-tile boundary when origins aren't tile-
  aligned) is separate, still-open follow-on work. And performance:
