...
- [x] **Widget-to-`Mesh` painting (`aurora-widgets::paint_widget`)** —
  ...
  **Scope, stated honestly**: `Button` only — a solid `rounded_rect`
  background (`scales.radius.sm`; no per-widget radius token exists in
  ...
  tokens throughout, invariant §7.3.10, nothing hardcoded. Every other
  `WidgetKind` (`Container`/`Checkbox`/`Slider`/`TextField`/
  `CommandPalette`) returns `Ok(None)` — a real, deliberate "no paint
  defined yet," not a placeholder rectangle; each needs its own shape
  designed first (a check glyph, a track + thumb, a caret, a list).
...
- [ ] Component gallery + golden-image tests per theme and density —
  **still genuinely blocked, though closer still**: `Button` can now go
  from layout bounds all the way to a real, theme-coloured `Mesh`
  (`paint_widget`, just above) and the GPU path renderer can draw that
  `Mesh` to real pixels — but nothing yet drives either of those over a
  real per-frame render pass covering a whole gallery of widgets in
  every forced state, and 8 of the 12 named widgets (everything but
  `Button`) have no paint defined at all yet. See
  [design/gallery/index.html](design/gallery/index.html) for the
  existing HTML review-surface skeleton this will eventually need to
  match, and `aurora-testkit`'s own `compare_to_golden` (M1.3) for the
  harness that will do the actual diffing once there's something real
  to render.
