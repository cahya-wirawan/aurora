...
- [~] **Component gallery + golden-image tests per theme and density** —
  ...
  **Still genuinely open**: the golden-blessing step above (human +
  real hardware); paint for the 8 of 12 named widgets that still have
  none (each needs the same "add a tree, call `render_gallery`, bless a
  golden" treatment once it has one); per-theme and per-density
  ...
  feed it, not a density axis); and matching
