...
- [~] **Component gallery + golden-image tests per theme and density** —
  ...
  **Still genuinely open**: two fresh bless steps (`command_palette`/
  `text_field`) waiting on Cahya, now against the corrected
  `NEUTRAL_CLEAR` backdrop; everything else this bullet's own earlier
  "still genuinely open" paragraph already named, unchanged.
