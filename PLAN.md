...
- [~] **Basic brush and eraser (real engine is Phase 2)** — first slice
  ...
  **What's still open**: which `aurora-doc` type owns a *live*
  `TileStore` instance (a new field on `LayerTree`, or a small wrapping
  type — ADR 0010 decided the storage model, not this); eraser (same
  `stamp_dab` math, transparent instead of `colour`, not built yet);
  undo-as-you-drag; wiring a live Brush/Eraser `aurora_ui::Tool` into
  `aurora-app`'s pointer input, which also needs `aurora-app` to hold a
  live document at all (it currently only builds one transiently in
  `App::new`). The Move tool's own blocker (active-layer selection) is
  unrelated to this ADR and remains separately open.
