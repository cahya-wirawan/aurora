...
- [ ] Component gallery + golden-image tests per theme and density —
  **still genuinely blocked, though closer**: needs a real widget-to-
  pixels rendering path to diff against a golden image at all. The GPU
  path renderer bullet just above closes the "how do these tessellated
  triangles actually become pixels" half; what remains is a real
  widget-to-`Mesh` painting path (no widget produces one yet — every
  widget still emits layout + accessibility content only) and the
  actual gallery test harness itself. See
