...
- [ ] Component gallery + golden-image tests per theme and density —
  **still genuinely blocked, though closer still**: a real per-frame
  render pass now draws every widget in a real tree to real pixels
  (`App::redraw`, just above) — but that pass lives in `aurora-app`,
  tied to a real window and event loop, not a headless harness a test
  could drive against an offscreen target and diff; building that
  harness, and designing paint for the 8 of 12 named widgets
  (everything but `Button`) that still have none, are what remain. See
