...
- [~] **Native menus, file dialogs, drag & drop, clipboard** — file
  dialogs, clipboard, and drag & drop done 2026-08-05; native menus
  ...
  **Still `[~]`, not `[x]`**: no real-hardware verification anywhere
  in this bullet (does the native "Open File…" dialog actually appear
  and behave correctly on each platform; does copy/paste actually
  round-trip through the real OS clipboard; does a real drag-and-drop
  gesture from the host OS actually reach `DroppedFile`); native menus
  (macOS menu bar, in particular — PRD explicitly calls this out) need
  a separate crate decision (`muda` is the likely candidate, not yet
  evaluated) — the one piece of this bullet still fully open.
