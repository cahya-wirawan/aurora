...
- [~] **Import/export PNG, JPEG, TIFF** — PNG done 2026-08-06; JPEG/TIFF
  still open. `aurora-io`'s first real code (was a placeholder). Asked
  ...
  **Still `[~]`, not `[x]`**: JPEG and TIFF remain unimplemented (each
  needs its own dependency decision); no wiring into a real document
  (no layer owns pixel storage yet, see above); no ICC-profile-aware
  import/export (always sRGB); no 16-bit export.
