...
- [~] **Import/export PNG, JPEG, TIFF** — all three done 2026-08-06,
  ...
  **PNG/TIFF ICC, PNG 16-bit export, JPEG quality, and TIFF float
  ...
  Genuinely still open: JPEG's own ICC/APP14 embedding (a segmented
  `APP2` marker, structurally different from PNG/TIFF's single-chunk
  case, not attempted); TIFF Palette/CMYK (needs a real ICC-aware
  `aurora_color::Transform`, not an uncalibrated formula); TIFF
  multi-page and compressed export.
...
- [~] **Format gaps: real ICC, 16-bit PNG, JPEG quality, TIFF float
  ...
  **Still `[~]`, not `[x]`**: JPEG's own ICC/APP14 embedding is a
  substantially harder, structurally different problem (a segmented
  `APP2` marker, not a single chunk) and wasn't attempted. TIFF
  Palette/CMYK still needs a real ICC-aware `aurora_color::Transform`,
  not an uncalibrated formula. TIFF multi-page and compressed export
  remain separate, still-open follow-on work, as does a real
  "this profile is exactly sRGB, use the lighter chunk" detection for
  PNG. `aurora-app` still has no colour-management UI or JPEG-quality
  UI to drive any of this with anything other than the sRGB/default-
  quality case in practice.
