...
- [~] **Native menus, file dialogs, drag & drop, clipboard** — file
  dialogs and clipboard done 2026-08-05; native menus and drag & drop
  still open. Picked the two remaining dependencies PRD §8.3/§14's own
  ...
  **Still `[~]`, not `[x]`**: no real-hardware verification (does the
  native "Open File…" dialog actually appear and behave correctly on
  each platform; does copy/paste actually round-trip through the real
  OS clipboard); native menus (macOS menu bar, in particular — PRD
  explicitly calls this out) need a separate crate decision (`muda` is
  the likely candidate, not yet evaluated); drag & drop is real,
  native-to-`winit` functionality (`WindowEvent::DroppedFile`/
  `HoveredFile`, no new dependency needed) but, like file-open, would
  have nothing real to hand a dropped path to yet — deferred alongside
  the rest of the real import pipeline, not forgotten.
