...
- [~] **Basic brush and eraser (real engine is Phase 2)** — first slice
  ...
  **Stops here, deliberately, not for lack of time**: the next piece —
  actually stamping a dab into pixels, and therefore eraser, undo-
  as-you-drag, and wiring a Brush/Eraser `aurora_ui::Tool` into
  `aurora-app`'s pointer input — needs a real place to write those
  pixels, and there isn't one. `aurora_doc::LayerKind::Pixel` has
  never owned a `TileStore`; whether pixel storage is one store per
  layer (simple, but an unlimited-layers document would mean an
  unlimited number of `TileStore`'s own background-writer threads) or
  one shared store addressed some other way has been an open, named
  question since M1.4 and was never answered. This is the same kind of
  fork the `.aur` format was before it got its own ADR (PRD §12 Q7) —
  a real architecture decision with lasting consequences, which
  CLAUDE.md is explicit belongs to Cahya to raise and decide, not
  something to invent silently as a side effect of finishing a basic-
  tools bullet. Flagged back rather than worked around; see PLAN.md's
  own "next step" note for this milestone.
